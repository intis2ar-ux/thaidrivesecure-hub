...
export const useApplications = () => {
  ...
  const generateAndStoreInsuranceDocument = async (
    ...
    // 2. Generate PDF
    const pdfBlob = generateInsurancePDF(application);

    // 3. Upload to Firebase Storage
    const fileName = `${Date.now()}.pdf`;
    const path = `insurance_documents/${application.id}/${fileName}`;
    ...
    await addDoc(collection(db, "orders", application.id, "status_logs"), {
      ...
      notes: `Insurance document generated by ${user.name || user.id}`,
